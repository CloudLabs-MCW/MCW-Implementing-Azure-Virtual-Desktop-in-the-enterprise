...
Once complete, select Next: Virtual Machine.
    ...
    ![This image shows the create a host pool blade, where you will enter in the information for the virtual machines that will host the remote apps and select next for workspace.](images/imagehostpool.png "Create host pool")

6.  When you configure **Virtual machine settings**, select **Browse all images and disks** and then select the tab option for **My Items** to select the image that was created earlier in **Exercise 4**.

    ![This image shows where you will find your custom image to add to the host pool.](images/imageselect.png "Host pool custom image")

    >**Note**: Selecting this image is very important. You will need the Microsoft 365 for assigning apps in this exercise.

7. Now, enter the rest of the settings for your host pool.
    ...
    - AD domain join UPN: avdadmin@contoso.com.
...
7. On the Workspace page, select **Yes** to register a new desktop app group. Select **Create new** and provide a **Workspace name** as **avdremoteapps**. Select **OK** and **Review + create**.
...
10. Open up **Active Directory Users and Computers** and find the **AVD App Hosts** security group we created earlier and switch to the **Members** tab.
    ...
    ![Azure Active Directory Users and computers with the **AVD App Hosts** group properties opened and teh **Members** tab selected.](images/avdapphostsgroup1.png)

11. Select **Add..**

    - Add **computers** to the **Object Types**.
    - Add the two virtual machines we just created **avdmcwpool-0; avdmcwpool-1**. Then select **OK**.
    - Select **OK**
    - Select **OK**
...
13. Wait a few minutes and make sure this change synchronizes to Azure Active Directory.
...
14. Reboot both of the virtual machines.
...
3.  Under Manage, select **Workspaces**. Locate the Workspace that was created for remote apps **avdremoteapps** and select the name.
...
3.  Under Manage, select **Host pools** and select the host pool that you created in Task 1 **avdremotepool1**.  Select **Application groups** and select **Add** to create a new application group.
...
4.  In the Basics tab, name the application group as **remoteapps**.
   
    ![This image shows that from this blade, you will enter a name for the application group.](images/avdapplicatingroup.png "New application group")
...
7. On the Add Application fly out, next to Application source, select **Start Menu**. Next, add the following applications, selecting **Save** between selections.
    ...
    ![This image shows the final list of applications.](images/avdworkshopadd.png "Application list")
