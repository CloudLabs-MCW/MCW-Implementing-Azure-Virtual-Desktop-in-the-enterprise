...
4.  On the Basics page, refer to the following screenshot to fill in the required fields. Change **Validation environment** to **Yes**. Once complete, select **Next: Virtual Machines**.
    ...
    ![This image shows where you will enter the information for the host pool.](media/hostpool.png "Create host pool page")
...
5.  Toggle **Add Azure virtual machines** to **Yes** and configure the Virtual Machines settings as follows:
   ...
   - Available options: No infrastructure redundancy is required.
   - Image: select a minimum of **Windows 10 Enterprise multi-session, Version 21H2 + Microsoft 365 Apps - Gen 1**
   - Virtual network: The AVD virtual network your other AVD Hosts are connected to.
   - AD domain join UPN: your avdadmin@contoso.com users.
...
6.  For the **Image**, select **Browse all images and disks** and search to find **Windows 10 Enterprise multi-session, Version 21H2 + Microsoft 365 Apps - Gen** and select that image.
...
7.  On the Workspace page, select **Yes** to register a new desktop app group. Select **Create new** and provide a **Workspace name** as **avdworkspace**. Select **OK** and **Review + create**.
...
5.  Under Manage, select **Assignments** and select **+ Add**.
    ...
    ![This image shows where to find manage in the menu and select assignments and add.](images/avdapplication.png)
